            ...
            <form action="{{ route('admin.showtimes.update', $showtime) }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <input type="number" step="0.01" class="form-control @error('price') is-invalid @enderror" 
                           id="price" name="price" value="{{ old('price', $showtime->price) }}" required>
                    @error('price')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="number" step="0.01" class="form-control @error('vip_price') is-invalid @enderror" 
                           id="vip_price" name="vip_price" value="{{ old('vip_price', $showtime->vip_price) }}" required>
                    @error('vip_price')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="alert alert-warning">
                    <i class="bi bi-exclamation-triangle"></i> Warning: Changing showtime details may affect existing bookings.
                </div>
